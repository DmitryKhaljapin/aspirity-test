...
import { PersonalInfoEidter } from '../';
...
export const PersonalInfo = ({ employeeData }: PersonalInfoProps): JSX.Element => {
    ...
    const [isEditingWindowOpened, setEditingWindowOpened] = useState<boolean>(false);
        ...
        <>
            <Card className={styles.card}>
                <div className={styles.header}>
                    ...
                    <button className={styles['edit-button']} onClick={() => setEditingWindowOpened(true)}>Изменить</button>
                </div>
            ...
            {isEditingWindowOpened && <PersonalInfoEidter employee={employeeData} closeHandler={() => setEditingWindowOpened(false)} />}
        </>
    );
}